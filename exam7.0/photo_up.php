        ...
        if (is_uploaded_file($_FILES['userfile']['tmp_name'])) {
            if (! @move_uploaded_file($_FILES['userfile']['tmp_name'], $_name)) {
                ...
                echo "<script>
                        ...
                        var img = window.opener.document.getElementById('url').value;
        	            var fm = window.opener.document.getElementsByTagName('form')[1];
                        alert('上传成功！'); 
                        fm.cont.value +='[img]'+img+'[/img]';
                        window.close();
                    </script>";
                exit();
            }
...
<body>
	...
	<div id="upimg" style="padding: 20px;">
		<form enctype="multipart/form-data" action="photo_up.php?action=up"
			method="post">
			<input type="hidden" name="MAX_FILE_SIZE" value="1000000" /> <input
				type="hidden" name="dir" value="<?php echo $_GET['dir']?>" /> 选择图片:
			<input type="file" name="userfile" /> <input type="submit" value="上传" />
		</form>
	</div>
</body>
</html>
