...
if ($_GET['action'] == 'register') {
	...
	$user_inf['identity'] = check_identity($_POST['identity']);
	...
	if (sql_affected_rows() == 1) {  //如果只改变了数据库中的一条语句则成功插入了
		...
		jump_page('恭喜您，注册成功！请登录。','login.php');
	} else {
		_Session_destroy();
		jump_page('注册失败！请重新注册。','register.php');
	}
...
<div id="member_main">
	<h2>用户注册  </h2>
	<form method="post" name="member_main" action="register.php?action=register">
		<input type="hidden" name="uniqid" value="<?php echo $uniqid ?>" />
		<dl>
			<dd class="face"><input type="hidden" name="face" value="face/t1.jpg" /><img src="face/t1.jpg" alt="头像选择" id="faceimg" /></dd>
			<dd style= "padding-right:40px">身　　份：<input type="radio" name="identity" value="0" checked="checked" />学生 <input type="radio" name="identity" value="1" />老师</dd>
